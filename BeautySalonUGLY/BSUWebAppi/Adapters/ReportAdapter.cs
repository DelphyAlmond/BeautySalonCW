    ...
    ILogger<ReportAdapter> logger, IMapper mapper) : IReportAdapter
{
    ...
    /// Получить отчёт в формате Word документа (.docx)
    public async Task<ReportOR> GetMasterVisitsReportWordAsync(string masterID, DateTime dateFrom, DateTime dateTo, CancellationToken ct)
    {
        try
        {
            // Валидация диапазона дат
            ...
            var fileName = $"Report_{masterName}_{dateFrom:yyyy-MM-dd}_{dateTo:yyyy-MM-dd}";

            _logger.LogInformation($"Word report generated successfully for master {masterID}");
            return ReportOR.OK(stream, fileName);
        }
        catch (ArgumentNullException ex)
        {
            _logger.LogError(ex, "ArgumentNullException");
        ...
        {
            _logger.LogError(ex, "Exception");
            return ReportOR.InternalServerError($"< Ошибка при генерации отчёта: {ex.Message} >");
        }
    }
